function control_NON_linear_model_V2(data_circadian,patient,theta_ott_NL,Ts,deltaT)
%% IMPORT CASADI
import casadi.*
...
for j=1:T
    V_traj = (xa(1,j) - 120)'*S*(xa(1,j) - 120) ;%+ (xa(4,j) - Si_Tar_eq(j))'*S*(xa(4,j) - Si_Tar_eq(j));
    %costo addizionale
...
%costo elemento T+1
V=V + (xa(1,T+1) - 120)'*S*(xa(1,T+1) - 120) + p_iper*delta_iper(T+1)^2 + p_ipo*delta_ipo(T+1)^2 + p_IOB*delta_IOB_eq(T+1)^2;
...
MPC=opti.to_function('MPC',{xk,rk,IOB_bound,Si_Tar_p,G_basal_p,IOB_basal_p,Si_Tar_eq,G_basal_eq,IOB_bound_eq},{uf,xf,V,xa,ua,delta_ipo,delta_iper,delta_ipo_x1,delta_iper_x1,delta_IOB,delta_IOB_eq},{'xk','rk','IOB_bound','Si_Tar_p','G_basal_p','IOB_basal_p','Si_Tar_eq','G_basal_eq','IOB_bound_eq'},{'uf_opt','xf_opt','V_opt','xa','ua','delta_ipo','delta_iper','delta_ipo_x1','delta_iper_x1','delta_IOB','delta_IOB_eq'});
...
for k=1:Tmax
    if mod(k-1,Ts) == 0
        ...
        [uf_sol,xf_sol,V_sol,xa_sol,ua_sol,d_ipo,d_iper,d_ipo_x1,d_iper_x1,d_IOB,d_IOB_eq]=MPC(xk_sim,rk_sim,IOB_vet(k:Ts:k+N*Ts),Si_tar(k:Ts:k+N*Ts-1),G_basal(k:Ts:k+N*Ts-1),IOB_basal,Si_tar(k:Ts:k+T*Ts-1),G_basal(k:Ts:k+T*Ts-1),IOB_vet(k:Ts:k+T*Ts));
